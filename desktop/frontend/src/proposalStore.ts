...
export const useProposalStore = create<ProposalState>((set, get) => ({
  ...
  async load(projectId) {
    ...
    if (!getter) { set({loading: false, busy: false, error: '桌面桥接尚未提供 Proposal API。', projectId, proposals: [], selected: null, diff: [], versions: []}); return }
    set({loading: true, error: '', notice: '', projectId, proposals: [], selected: null, diff: [], versions: []})
    const current = () => generation === projectGeneration && sameProject(get().projectId, projectId)
    try {
    ...
    } catch (cause) { if (current()) set({error: String(cause)}) }
    finally { if (current()) set({loading: false}) }
  },
  async select(id) {
    ...
    if (!getter) return
    const projectAtStart = get().projectId
    const generation = projectGeneration
    const current = () => generation === projectGeneration && sameProject(get().projectId, projectAtStart)
    set({loading: true, error: ''})
    try {
      const selected = await getter(id)
      if (!current() || (get().projectId && !sameProject(selected.projectId, get().projectId))) return
  ...
  async createManual(chapter, title, after) {
    ...
    const generation = projectGeneration
    const current = () => generation === projectGeneration && sameProject(get().projectId, projectIdAtStart)
  ...
  async accept() {
    ...
    const selected = get().selected
    if (!action || !selected || get().busy) return
    const projectIdAtStart = get().projectId
    const generation = projectGeneration
    const current = () => generation === projectGeneration && sameProject(get().projectId, projectIdAtStart)
    set({busy: true, error: '', notice: ''})
    try { const updated = await action(selected.id); if (!current()) return; set({selected: updated, proposals: get().proposals.map(item => item.id === updated.id ? updated : item), notice: '建议已接受；正文尚未修改。'}) }
    catch (cause) { if (current()) set({error: String(cause)}) }
    finally { if (current()) set({busy: false}) }
  },
  async reject() {
    ...
    const current = () => generation === projectGeneration && sameProject(get().projectId, projectIdAtStart)
    set({busy: true, error: '', notice: ''})
    try { const updated = await action(selected.id); if (!current()) return; set({selected: updated, proposals: get().proposals.map(item => item.id === updated.id ? updated : item), notice: '建议已拒绝。'}) }
    catch (cause) { if (current()) set({error: String(cause)}) }
    finally { if (current()) set({busy: false}) }
  },
  async apply() {
    ...
    const selected = get().selected
    if (!action || !selected || get().busy) return null
    const projectIdAtStart = get().projectId
    const generation = projectGeneration
    const current = () => generation === projectGeneration && sameProject(get().projectId, projectIdAtStart)
    set({busy: true, error: '', notice: ''})
  ...
  async restoreVersion(id) {
    ...
    try {
      ...
      if (statusGetter) {
        ...
        if (current() && sameProject(status.projectId, projectIdAtStart)) useRevisionStore.getState().acceptStatus(status)
      }
