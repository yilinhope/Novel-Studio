...
import { shouldOfferImmediateChapterSync, useRevisionStore } from '../revisionStore'

export function ChapterReader() {
  const {chapter, chapterLoading, error, draftContent, dirty, saveBusy, syncing, saveError, syncError, syncNotice, refreshWarning, setDraftContent, saveChapter, syncChapterRevisions} = useStudio()
  const runtime = useEngineStore(state => state.runtime)
  const revisions = useRevisionStore(state => state.status)
  const revisionChecking = useRevisionStore(state => state.checking)
  ...
  const canSyncChapter = shouldOfferImmediateChapterSync(revisions.state, unsynced, dirty)
  ...
  return <article className="reader"><p className="eyebrow">CHAPTER {String(chapter.number).padStart(3,'0')} / 章节正文</p>
    ...
    <div className="editor-toolbar">
      ...
      {canSyncChapter && <button disabled={syncing || saveBusy || runtimeReadOnly || revisionChecking} title="同步项目中所有已保存的章节修订" onClick={() => void syncChapterRevisions()}>{syncing ? '同步中…' : '立即同步'}</button>}
    ...
    {syncError && <p className="editor-error" role="alert">章节修订同步失败：{syncError}</p>}
    {syncNotice && <p className="editor-hint" role="status">{syncNotice}</p>}
    {refreshWarning && <p className="editor-hint" role="status">同步成功，但视图刷新提醒：{refreshWarning}</p>}
