...
function StylePanel() {
  const {style, selectStyle, saveAsset} = useWritingSettingsStore()
  const [voice, setVoice] = useState(style?.voice ?? '')
  const [anti, setAnti] = useState(style?.antiAiTone ?? '')
  const [scope, setScope] = useState<'global' | 'project'>('project')
  useEffect(() => { setVoice(style?.voice ?? ''); setAnti(style?.antiAiTone ?? '') }, [style?.voice, style?.antiAiTone])
  if (!style) return <div className="parity-empty"><span>—</span><p>Core 尚未提供文风状态。</p></div>
  return <div><div className="parity-card"><h2>Style</h2><select value={style.selectedStyle} onChange={event => void selectStyle(event.target.value)}>{style.styleNames.map(name => <option key={name} value={name}>{name}</option>)}</select><p className="muted">来源：{style.styleSource}</p><p className="long-text">{style.selectedStyleText || 'Core 未提供当前 Style 文本。'}</p>{style.genreReference && <><p className="muted">题材参考来源：{style.genreReferenceSource}</p><p className="long-text">{style.genreReference}</p></>}</div><div className="parity-card"><label>保存覆盖范围 <select value={scope} onChange={event => setScope(event.target.value as 'global' | 'project')}><option value="project">项目</option><option value="global">全局</option></select></label></div><div className="wave-columns"><AssetEditor title="Voice" source={style.voiceSource} value={voice} onChange={setVoice} onSave={() => void saveAsset(scope, 'voice.md', voice)}/><AssetEditor title="Anti-AI-Tone" source={style.antiAiToneSource} value={anti} onChange={setAnti} onSave={() => void saveAsset(scope, 'anti-ai-tone.md', anti)}/></div><p className="muted">{style.effectiveNotice}</p></div>
}

function AssetEditor({title, source, value, onChange, onSave}: {title: string; source: string; value: string; onChange: (value: string) => void; onSave: () => void}) { return <div className="parity-card rule-editor"><h2>{title}</h2><p className="muted">来源：{source}</p><textarea value={value} onChange={event => onChange(event.target.value)} rows={10}/><button className="primary" onClick={onSave}>保存 {title}</button></div> }
