...
export function CreateCenter() {
  ...
  return <section style={cardStyle}>
    ...
    {state.mode === 'cocreate' && state.coCreate && <div className="panel" style={{marginTop: 20}}><h3>Core 共创回复</h3><p style={{whiteSpace: 'pre-wrap'}}>{state.coCreate.reply || state.coCreate.text || 'Core 正在整理…'}</p>{state.coCreate.draft && <details><summary>当前创作指令草稿</summary><pre className="prose">{state.coCreate.draft}</pre></details>}<div style={{display: 'flex', gap: 8, marginTop: 12}}><input value={sent} onChange={e => setSent(e.target.value)} placeholder="继续告诉 Core 你的想法"/><button disabled={state.busy || !sent.trim()} onClick={() => {void state.send(sent); setSent('')}}>发送</button>{state.coCreate.ready && <button disabled={state.busy} onClick={() => void state.complete()}>完成并开始创作</button>}</div></div>}
    {state.recovery?.exists && <div className="panel" style={{marginTop: 20}}><h3>发现已有共创会话</h3><p>{state.recovery.interrupted ? '上一轮未完成，Core 已保留可恢复的最后一轮。' : 'Core 已保留最近共创记录。'}</p><small>{state.recovery.history?.length ?? 0} 条消息</small></div>}
    {state.event?.error && <div className="editor-error">创建失败：{state.event.error}</div>}
  </section>
}

export function ImportCenter() {
  const {open} = useStudio(); const state = useImportStore(); const [options, setOptions] = useState({projectRoot: '', sourcePath: '', autoConfirm: false, acceptSegmentation: false, storyResolution: '', continueAfter: false, guidance: ''})
  useEffect(() => { void state.refresh() }, [])
  return <section style={cardStyle}><p className="eyebrow">IMPORT NOVEL</p><h1>导入已有小说</h1><p className="muted">阶段、章节识别、确认、分析、综合与发布全部来自 Core Import Pipeline。</p><form onSubmit={e => {e.preventDefault(); void state.start(options)}} style={{display: 'grid', gap: 12}}><label>新项目根目录（已有项目可留空）<input value={options.projectRoot} onChange={e => setOptions({...options, projectRoot: e.target.value})} placeholder="例如 D:\\小说\\imported"/></label><label>源文件路径<input value={options.sourcePath} onChange={e => setOptions({...options, sourcePath: e.target.value})} placeholder="TXT / EPUB 等 Core 支持的文件" required/></label><label>切分指导（Core 原生 guidance）<input value={options.guidance} onChange={e => setOptions({...options, guidance: e.target.value})}/></label><div><label><input type="checkbox" checked={options.acceptSegmentation} onChange={e => setOptions({...options, acceptSegmentation: e.target.checked})}/> 已查看并接受章节识别结果</label><label><input type="checkbox" checked={options.continueAfter} onChange={e => setOptions({...options, continueAfter: e.target.checked})}/> 导入完成后按 Core 语义继续创作</label></div><button className="primary" disabled={state.busy}><FolderInput size={15}/>{state.busy ? 'Core 正在导入…' : '开始导入'}</button></form>{state.busy && <button onClick={() => void state.cancel()}>取消当前导入</button>}{state.error && <div className="editor-error">{state.error}</div>}{state.status && <div className="panel" style={{marginTop: 20}}><h3>{state.status.stage || '等待 Core'}</h3><p>{state.status.message || 'Core 正在读取导入工作区。'} {state.status.current || 0}/{state.status.total || 0}</p>{state.status.recoveryHint && <p className="muted">恢复提示：{state.status.recoveryHint}</p>}{state.status.chapters?.map(chapter => <div key={chapter.number}>{chapter.number}. {chapter.title} {chapter.uncertain ? ' · 需确认' : ''}</div>)}{state.status.stage === 'done' && <button onClick={() => void open(state.status?.projectId)}>打开导入结果</button>}</div>}</section>
}
...
export function SettingsCenter() {
  ...
  return <section style={cardStyle}>
    ...
    </> : <p className="muted">正在读取 Core effective config…</p>}
  </section>
}
