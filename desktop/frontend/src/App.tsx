...
import { Activity, BookOpen, FolderOpen, LayoutDashboard, RefreshCw, Library, ShieldCheck } from 'lucide-react'
...
export default function App() {
  const {project, chapter, busy, error, view, dirty, open, overview, runtime} = useStudio()
  ...
  return <div className="studio">
    ...
    <footer><span className={`status-dot runtime-state-${engine.state}`}/>{project ? runtimeLabel(engine.state) : '只读浏览'}<span className="footer-right">{project ? `${project.overview.completedChapters} 章已完成 · ${project.overview.wordCount.toLocaleString()} 字` : '尚未打开项目'}</span><span>Novel Studio · M4-B</span></footer>
  </div>
}
