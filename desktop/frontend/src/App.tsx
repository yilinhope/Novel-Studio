...
import { Activity, ArrowDownToLine, BookOpen, ClipboardCheck, FilePlus2, FolderInput, FolderOpen, LayoutDashboard, Library, Play, RefreshCw, Settings2, ShieldCheck } from 'lucide-react'
...
export default function App() {
  const {project, chapter, busy, error, view, dirty, open, overview, runtime, review, create, imports, settings, exports} = useStudio()
  ...
  return <div className="studio">
    ...
    <footer><span className={`status-dot runtime-state-${engine.state}`}/>{project ? runtimeLabel(engine.state) : '只读浏览'}<span className="footer-right">{project ? `${project.overview.completedChapters} 章已完成 · ${project.overview.wordCount.toLocaleString()} 字` : '尚未打开项目'}</span><span>Novel Studio · M6</span></footer>
  </div>
}
