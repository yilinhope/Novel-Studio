...
export default function App() {
  const {project, chapter, busy, error, view, open, overview, runtime} = useStudio()
  ...
  return <div className="studio">
    ...
    {project ? <div className="workspace">
      ...
      <main><div className="tabbar"><span>{view === 'overview' ? '作品总览' : view === 'runtime' ? '运行中心' : '章节正文'}</span><span className="muted">{view === 'chapter' && chapter ? `第 ${chapter.number} 章` : '本地项目'}</span></div><div className="canvas">{view === 'overview' ? <Overview data={project.overview}/> : view === 'runtime' ? <RuntimeCenter/> : <ChapterReader/>}</div></main>
    ...
    <footer><span className={`status-dot runtime-state-${engine.state}`}/>{project ? runtimeLabel(engine.state) : '只读浏览'}<span className="footer-right">{project ? `${project.overview.completedChapters} 章已完成 · ${project.overview.wordCount.toLocaleString()} 字` : '尚未打开项目'}</span><span>Novel Studio · M3-B</span></footer>
  </div>
}
